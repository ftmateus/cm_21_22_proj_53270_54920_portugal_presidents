...
void Gallery::drawStringCentered(const std::string& c, float x, float y)
{
...
void Gallery::drawStringRight(const std::string& c, float x, float y)
{
    ofRectangle stringBox = myfont.getStringBoundingBox(c, 0, 0);
...
void Gallery::drawVideo(ofVideoPlayer *vid) {
...
bool Gallery::isMousePtrInCarrousel(int x, int y)
{
...
//--------------------------------------------------------------
void Gallery::mouseScrolled(int x, int y, float scrollX, float scrollY)
{
    if (isMousePtrInCarrousel(x, y))
    {
...
//--------------------------------------------------------------
void Gallery::windowResized(ofResizeEventArgs& resize){
...
//--------------------------------------------------------------
string Gallery::edgesFilter(string presidentName, ofImage image) {
    double kernel_size;
    Mat kernel;
    kernel_size = 31;
    kernel = Mat::ones(kernel_size, kernel_size, CV_64F) / (kernel_size * kernel_size);
...
string Gallery::textureFilter(string presidentName, ofImage image) {
    Mat src, dst;
    int kernel_size = 31;
    ...
    filter2D(src, dst, CV_64F, kernel);
...
void Gallery::generateMetadata(string presidentName, string path, ofImage image, bool isVideo) {
